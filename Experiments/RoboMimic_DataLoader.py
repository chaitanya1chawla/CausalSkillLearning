...
class Robomimic_Dataset(Dataset):
	...
	def setup(self):
		...
		for i in range(len(self.task_list)):
			...
			self.files.append(h5py.File("{0}/{1}/low_dim.hdf5".format(self.dataset_directory,self.task_list[i]),'r'))
	...
	def preprocess_dataset(self):
		...
		for task_index in range(len(self.task_list)):
			...
			for i in range(1,self.num_demos[task_index]+1):
				...
				joint_action_sequence = resample(joint_action_sequence, robot_state_sequence.shape[0]//self.ds_freq)
				gripper_action_sequence = resample(gripper_action_sequence, robot_state_sequence.shape[0]//self.ds_freq)
