...
for i in range(number_datapoints):
	...
	for t in range(number_timesteps-1):
		...
		if t>0:
			...
			if reset_counter>=2 and reset_counter<4:
				b_array_dataset[i,t] = np.random.binomial(1,0.33)
			elif reset_counter==4:
...
np.save("X_GMM_Toy.npy",x_array_dataset)
np.save("Y_GMM_Toy.npy",y_array_dataset)
np.save("B_GMM_Toy.npy",b_array_dataset)
